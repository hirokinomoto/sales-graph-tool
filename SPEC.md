...
本ドキュメントは、提出用の最小プロダクトとして「売上CSV → カテゴリ別売上合計の棒グラフ（画面表示）」を実現するTkinterアプリの仕様（正本）である。

- 仕様の正本：本ドキュメント（SPEC.md）
- 決定の正本：決定事項ログ（ssot/決定事項ログ.md）
...
非エンジニア（例：店舗責任者/マーケ担当）が、カテゴリ別の売上規模をざっくり把握したい。
...
非エンジニアの担当者として、売上CSVを選んでグラフを作成し、カテゴリ別売上規模を把握したい。
...
売上CSVからカテゴリ別売上を自動集計し、ワンクリックで棒グラフ化して確認できる最小ツールです。非エンジニアでも手早く売上規模を把握できます。
